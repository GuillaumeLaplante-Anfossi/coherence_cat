...
In other words, the  proof that Kapranov's conditions imply Mac Lane's conditions  is non-trivial, in contrast to the converse direction (cf. \cref{Kapranov-MacLane}), but rather a result of the magic of polytopes!
\end{rem}}
\correction{\begin{rem}
\label{MacLane-Kapranov-Simple}
    Note that contrary to the case of operahedra, there does not seem to exist a generic vector inducing an orientation of the edges of the permuto-associahedra which coincides with a consistent orientation of the $\beta$ and $\tau$ arrows, based on  conventions independent of the orientation vector (say, move parentheses to the right for $\beta$ as done here, and orient $\tau$ according to a global order on $S$, and moving the maximum to the right as in the weak Bruhat ordering), as the dodecagon (\cref{fig:dodecagon}, left) involves $\beta^{-1}$ arrows.  The same remarks apply to the simple permutoassociahedra of \cite{baralicSimplePermutoassociahedron2019} and also to the case of the original presentation of Mac Lane. For the latter, no polytopal correspondence is known, but one could still consider the associated abstract rewriting system.  Mac Lane's proof proceeds in two stages: rewriting for the monoidal part, and then dealing with the symmetric part using Coxeter's presentation of the symmetric group. It seems that one cannot do better. Indeed, even if  Mac Lane's hexagon as described above does not involve   $\beta^{-1}$  arrows, the latter pop up when taking into account  the combinatorics of orientation of the $\tau$ arrows.
\end{rem}}
