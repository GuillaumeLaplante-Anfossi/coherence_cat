...
\correction{
In order to state the coherence theorem, we construct the free symmetric monoidal category on a set $S$ of \defn{generating objects}. 
...
\end{enumerate}
We can see the objects of $\mathcal{S}^{\mathrm{ML}}$ as (fully) parenthesized words over $A$ where letters are not repeated.
...
\end{quote}
In other words, hexagons are now taken as definitions rather than axioms. But how do we guarantee that the  general $\tau$ morphisms defined in this way define a natural transformation? This is what the dodecagons are for, as we will now see.}
...
Given a finite set $A$ and a parenthesized permutation $\sigma$ of the its $n$ elements, one associates a section $\gamma^\sigma$ of the projection from the $n$-cube to the cyclic polygon with $n+1$ vertices \cite[Ex.~9.14]{Ziegler95}, whose integral over the base gives a point~$M(\sigma)$ in $\R^n$. 
...
\correction{Parenthesized ordered partitions of $A$ can be drawn as planar trees whose leaves are parts of a partition of $A$.
