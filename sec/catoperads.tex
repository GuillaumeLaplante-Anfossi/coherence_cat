...
\begin{definition}
    We denote by $\mathcal{F}(S)$ the quotient of the family of categories $\mathcal{S}$ by localization (inverting the $\beta$ and $\theta$ morphisms), the axioms of bifunctors for the $\circ_i$, \correction{the naturality conditions for $\beta$ and $\theta$}, and the coherence diagrams (i.e. the pentagons and hexagons) defining a categorified non-symmetric operad. 
\end{definition}
...
Given a planar tree $t$ with $n$ internal edges, and a full nesting $\mathcal{N}$ of $t$, one associates a point~$M(t) \in \R^n$ via a Loday-type algorithm \cite[Sec.~2.2]{laplante-anfossiDiagonalOperahedra2022a}.  
The operahedron $P_t \subset \R^n$ is the convex hull of the points $M(t)$, for all maximal nestings of $t$.
It has dimension $n-1$. 
One then shows \cite[Prop.~2.15]{laplante-anfossiDiagonalOperahedra2022a} that the poset of nestings of $t$, ordered by de-refinement, is isomorphic to the poset of faces of $P_t$. 
...
\label{prop:bijection-nestings}
    ...
    \begin{enumerate}
        \item objects of $\calS$ and vertices of the operahedra,
        \item generating morphisms of $\calS$ and edges of the operahedra,
        \item coherence, bifunctoriality and naturality diagrams and $2$-faces of the operahedra,
    \end{enumerate}
    with one operahedron for each planar tree whose vertices are decorated by elements of $S$.
\end{proposition}
\begin{proof}
    To each element $\mu$ of $S_n$, we associate a planar corolla with $n$ leaves and vertex decorated by $\mu$. 
Then, we identify $\mu \circ_i \nu$ with the planar tree obtained from grafting the corolla decorated by $\nu$ at the $i$th leaf of the corolla decorated by $\mu$.
...
\begin{proof}[Proof of {\cref{thm:coherence-operahedra}}]
    \correction{From Point (2) in \cref{prop:bijection-nestings}, we have that} the morphisms of $\mathcal{S}$ are in bijection with combinatorial paths on the operahedra.
...
The result then follows from \correction{Point (3) in \cref{prop:bijection-nestings}, that is} the fact that a $2$-face of an operahedron is exactly either a square (witnessing naturality \correction{or bifunctoriality}), a pentagon or an hexagon (witnessing a coherence condition) as in \cref{def:catoperad}.
\end{proof}
...
\begin{proof}[Second proof of {\cref{thm:coherence-operahedra}}]
    ...
    The \correction{lattices obtained on fully nested trees} generalize the Tamari lattice, \correction{which corresponds to} fully nested \emph{linear} trees \cite{DefanSack24}.
    One then obtains a topological proof of coherence which \correction{follows closely} the original proof of MacLane \cite[Thm.~3.1]{MacLane63}, suitably generalized to categorified operads. 
\end{proof}
