...
Iterated applications of the $\circ_i$ can be visualized as fully nested planar trees \cite[Definition 2.2]{laplante-anfossiDiagonalOperahedra2022a}, with vertices decorated by objects of $\mathcal{P}$, see \cref{fig:treeandnesting}. 
...
\begin{rem}
\label{rem:DPLA}
K. Do{\v s}en and Z. Petri{\'c} introduced in \cite[Section 12]{DP15} the notion of weak Cat-operad.
...
\end{enumerate}
Note that by construction, for every morphism $\phi : t_1 \to t_2$, the objects $t_1$ and $t_2$ have the same arity, and we say that $\phi$ has this \emph{arity}. 
