...
These will be associated with the local confluence diagrams of a rewriting system on constructions in \cref{ss:critical}.
...
\begin{itemize}
\item[(A)]  $T$ has exactly two non-singleton nodes $\set{x_1,x_2}$ and $\set{y_1,y_2}$, both of cardinality $2$. 
\item[(B)] $T$ has exactly one non-singleton node $\set{x_1,x_2,x_3}$ of cardinality $3$.
\end{itemize}
...
\xymatrix @-1.65pc { x_1(x_2)\cdots y_1(y_2) \ar @{-}[dddd]_{x_1(x_2)\cdots \set{y_1,y_2}} 
 ...
 $$
The construct $T$ which corresponds to the 2-face above has two distinct non-singleton nodes $\set{x_1,x_2}$ and $\set{y_1,y_2}$, whence the schematized expression  $\set{x_1,x_2} \cdots \set{y_1,y_2}$.
...
Like in the previous case, the constructs of the edges and vertices of the $2$-face corresponding to~$T$ result from replacing in $T$ the node $X$ with the indicated respective trees and redistributing uniquely the  children of $X$ (see also~\cref{instance-construct}).
...
By~\cref{xyz-reconnected}, we can read these pictures as describing the 
...
Incidentally, these  hypergraphs witness the fact that there do exist 2-faces of type (B) of  each  of these types: take $\hyper{H}$ to be one of those four $2$-dimensional hypergraphs, and $T$ to be their unique construct of dimension 2.
...
For a 3-element subset $X=\set{x_1,x_2,x_3}$ of $H$, we say that a 2-face viewed as a contruct $T$ of $\hyper{H}$ is an \defn{$X$-face} if its unique non-singleton node is decorated by $X$.  By letting $X$ range over all subsets of $H$ of cardinality 3, we form in this way a partition of all 2-faces of type (B).
If $X$ is the root of $T$, the following  lemma invites us to see $T=X(\ldots)$ as an ``instantiation'' of~$X$ (viewed as the maximum face of $\recrestr{\hyper{H}}{X}$).

\begin{lemma} 
  \label{instance-construct} 
  If $\hyper{H}$ is a connected hypergraph, if $X$ is a subset of $H$ such that $|X|=3$ and~$T$ is a 2-dimensional construct with root $X$, then the map $\psi$ from the poset of subfaces of $T$ to the poset of faces of $\recrestr{\hyper{H}}{X}$, defined on a face $S$ by pruning all nodes of $S$ that are not subsets of $X$, is an order-isomorphism.
\end{lemma}
